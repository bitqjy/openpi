...
class Policy(BasePolicy):
    def __init__(
        ...
        *,
        ...
        if self._return_frozen_action_latent and self._return_safe_pre_velocity:
            raise ValueError("Frozen RL latent and SAFE pre-velocity modes are mutually exclusive")
    ...
    def infer(self, obs: dict, *, noise: np.ndarray | None = None) -> dict:  # type: ignore[misc]
        ...
        if safe_pre_velocity is not None:
            outputs["safe_pi0_pre_velocity"] = np.asarray(safe_pre_velocity[0, ...], dtype=np.float32)
